...
As a case study we consider Tassel-earred Squirrels (\emph{Sciurus aberti}), a species
currently distributed in disjunct patches of pine forest from the southern Rocky
Mountains in the United States to the northern Sierra Madre Occidental
in Mexico for which reliable population-genetic and occurrence information is available. Our results reveal that the genetic fixation
index (\(F_{st}\)) of \emph{S. aberti} populations with respect to a source
population correspond to the area of greatest prevalence through time, as modeled over the last 120,000 years. Populations closer to the historical endemic area display a
higher genetic diversity and a lower \(F_{st}\) value. This result suggests a general biogeographic explanation for the common observation of geographic clines of genetic diversity within species. By generating a time series of habitat suitability maps for a given species,  the suitability prevalence area can be estimated. Thus, integrating  population-genetic data with estimates of historical patterns of species
distribution holds promise for improving conservation and management efforts.

\hypertarget{introduction}{%
\subsection{Introduction}\label{introduction}}

The current distributions of species may not correspond to their
historical distributions over evolutionary significant time periods. This
is because environmental conditions are not static over time, and the populations of a species
tend to 'track' environments, distributing where conditions are most favorable in the absence of barriers (REF).
...
We define \(\vec{e}_i =(e_1, e_2, ..., e_n )\) as a vector of
environmental variables, that is, a point in the space of \(n\)
...
is \(\mathbf{N}_F = \{\vec{e} | R_0(\vec{e}(t)) > k \}\).
...
populations and positive rates. We assume that the environment
...
\(N_i(t+1) = R_0(\vec{e}_i(t) ) N_i(t)\). That is, the locally effective
...
environmental space. Considering this rate fixed \(R_0\) when
...
This equation was proposed by Soberon and Peterson
...
\(S_i(\vec\{e\}\_i)\) of environmental suitability that can be
